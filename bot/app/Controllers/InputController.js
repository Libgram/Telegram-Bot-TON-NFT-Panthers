...
class InputController {
	...
	static async setAddressName({ctx, path}) {	
		try{

			if (ctx.userInfo.tutorial === -1) {
				let input = ctx.message.text;
				if (input.length > 20) throw {status: false, id: 1};

				let address = await Address.get(path.type);
				if (address) {
					address.name = input;
					await address.save();
					...
					let setAddressNameDoneTemplate = templates.setAddressNameDone(ctx);
					await ctx.reply(setAddressNameDoneTemplate.text, setAddressNameDoneTemplate.extra);
					return;
			...
			if (e.status === false) {
				...
				switch (e.id) {
					case 1:
				 	await ctx.reply(ctx.i18n.__('errors.lengthString'));
					return;
